...
def dense_unit_output(input_tensor, nn):
    # without activation function and dropout
    out_tensor = BatchNormalization()(input_tensor)
    out_tensor = Dense(nn)(out_tensor)
    return out_tensor
...
def bfnn_model_est():  # construct the BFNN proposed in this letter
    input_tensor = Input(shape=(2*Nt+1,))  # consists of the estimated CSI and the SNR value
    temp = dense_unit_dropout(input_tensor, 4096, 0.45)  # the first dense unit with 2048 neurons
    temp = dense_unit_dropout(temp, 2048, 0.35)  # the second dense unit with 1024 neurons
    temp = dense_unit_dropout(temp, 1024, 0.3)  # the third dense unit with 256 neurons
    temp = dense_unit_dropout(temp, 256, 0.2)  # the third dense unit with 256 neurons
    out_phases = dense_unit_output(temp, Nt)  # to output the phases of v_RF
...
def bfnn_model():  # construct the BFNN proposed in this letter
    input_tensor = Input(shape=(2*Nt+1,))  # consists of the estimated CSI and the SNR value
    temp = dense_unit_dropout(input_tensor, 2048, 0)  # the first dense unit with 2048 neurons
    temp = dense_unit_dropout(temp, 1024, 0)  # the second dense unit with 1024 neurons
    temp = dense_unit_dropout(temp, 256, 0)  # the third dense unit with 256 neurons
    out_phases = dense_unit_dropout(temp, Nt, 0)  # to output the phases of v_RF
    model = Model(input_tensor, out_phases)
    model.compile(optimizer=tf.train.AdamOptimizer(), loss=loss_function_rate)
    print(model.summary())
    return model
